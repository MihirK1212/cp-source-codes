...
```
nx^2 −2x(a1 + a2 +··· + an)+(a^2_1 + a^2_2 +··· + a^2_n)
```

The last part does not depend on `x`, so we can ignore it. The remaining parts form a function `nx^2 − 2xs` where `s = a1 + a2 +··· + an`. This is a parabola opening upwards with roots `x = 0` and `x = 2s/n`, and the minimum value is the average of the roots `x = s/n`, i.e., the average of the numbers `a1, a2, ..., an`.